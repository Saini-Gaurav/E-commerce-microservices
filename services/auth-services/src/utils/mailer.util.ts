...
export async function sendPasswordResetOtpEmail(toEmail: string, otp: string): Promise<void> {
  await transporter.sendMail({
    from: `"Herbsvedic" <${process.env.EMAIL_USER}>`,
    to: toEmail,
    subject: "Reset your Herbsvedic password",
    html: `
      <div style="font-family: sans-serif; max-width: 480px; margin: auto;">
        <h2 style="color: #3F5233;">Reset your password</h2>
        <p>Your one-time verification code is:</p>
        <p style="font-size: 32px; font-weight: bold; letter-spacing: 8px; color: #1F2A1C;">${otp}</p>
        <p style="color: #666; font-size: 13px;">This code expires in 10 minutes. If you didn't request a password reset, you can safely ignore this email - your password has not been changed.</p>
      </div>
    `,
  });
}