...
// The shape actually sent to clients: price/rating as real numbers (see
// note in product.repository.ts on why pg gives us strings), and
// category_id -> categoryId to keep the API surface camelCase even
...
async function assertCategoryExists(categoryId: string): Promise<void> {
  ...
  if (!category) {
    ...
    throw new ServiceError("categoryId does not reference an existing category", 400);
  }
}

export async function createProduct(input: CreateProductInput): Promise<ProductResponse> {
  // Cross-table check that lives in the SERVICE layer, not the
  // repository or a DB-level FK, because categories and products are
  // technically in the same database here - but the moment this
  // architecture splits categories into their own service, this
  // exact validation becomes an HTTP call instead of a local query,
  // and business logic (services), not data access (repositories), is
...
export async function updateProduct(
  ...
  // Repository update fn takes snake_case DB column keys; translate here
  // at the service boundary so the repository stays a dumb mirror of
  // the table, and callers of THIS function keep using camelCase.
  const dbFields: Record<string, unknown> = {};
  if (fields.name !== undefined) dbFields.name = fields.name;
  if (fields.description !== undefined) dbFields.description = fields.description;
  if (fields.richDescription !== undefined) dbFields.rich_description = fields.richDescription;
  ...
  if (fields.ingredients !== undefined) dbFields.ingredients = fields.ingredients;
  ...
  if (fields.precautions !== undefined) dbFields.precautions = fields.precautions;
