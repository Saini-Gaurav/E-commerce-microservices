...
export async function listProductsHandler(req: Request, res: Response): Promise<void> {
  try {
    const { categoryId, isFeatured, search, page, limit } = req.query;
...
export async function createProductHandler(req: Request, res: Response): Promise<void> {
  try {
    const {
      name, description, richDescription, image, images, brand, price,
      categoryId, countInStock, isFeatured, ingredients, usageNotes,
      benefits, precautions, quantity,
    } = req.body;

    if (!name || !description || price === undefined || !categoryId || countInStock === undefined) {
      res.status(400).json({
        message: "name, description, price, categoryId, and countInStock are required",
      });
      return;
    }

    const product = await productService.createProduct({
      name, description, richDescription, image, images, brand,
      price: Number(price),
      categoryId,
      countInStock: Number(countInStock),
      isFeatured, ingredients, usageNotes, benefits, precautions, quantity,
    });
